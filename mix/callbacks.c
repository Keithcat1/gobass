...
#include <stdlib.h>
CALLBACK void _MixSyncprocCallbackWrapper(HSYNC sync, HCHANNEL channel, DWORD data, void* userdata) {
	_MixSyncprocCallback(sync, channel, data, userdata);
}
...
extern SYNCPROC* _get_MixSyncprocCallbackWrapper() {
	return _MixSyncprocCallbackWrapper;
}